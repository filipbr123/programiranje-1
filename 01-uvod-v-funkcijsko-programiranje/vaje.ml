...
let sestej vek1 vek2 =
  let pari = List.combine vek1 vek2 in
...
let skalarni_produkt v1 v2 = 
  ...
  let nov = List.map (fun (c1, c2) -> c1 *. c2) pari in
  vsota_seznama nov
...
let norma vektor = 
  let vektor2 = List.map (fun x -> x *. x) vektor in
  sqrt(vsota_seznama vektor2)
